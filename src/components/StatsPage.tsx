        ...
        {filteredStats.length === 0 ? (
            ...
            {filteredStats
              ...
              .map((s) => (
                <div key={s.playerId} className="bg-surface-800 border border-surface-600/50 rounded-xl p-3 animate-fade-in">
                  ...
                  {/* Per-fundamental bars with bold Eff% and Pos% */}
                    ...
                    {(['ATT', 'RIC', 'BAT', 'MUR', 'DIF'] as Fundamental[]).map((f) => {
                      ...
                      return (
                        <div key={f} className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-extrabold text-white w-10 text-right">{fs.efficiency}%</span>
                          <span className="text-[10px] font-extrabold text-blue-400 w-10 text-right">{fs.positivity}%</span>
                        </div>
                      );
                    })}
                  </div>
          ...
          <div className="mt-2">
            <div className="bg-surface-800 border border-surface-600/50 rounded-xl overflow-hidden">
              {/* Table header */}
              <div className="grid grid-cols-[32px_1fr_32px_32px_32px_32px_36px_44px_44px] gap-0 px-2 py-2 bg-navy-800/50 text-[10px] font-bold text-muted-dark uppercase tracking-wider">
                <span>N°</span>
                ...
                <span className="text-center font-extrabold">{t.stats_efficiency}</span>
                <span className="text-center font-extrabold">{t.stats_positivity}</span>
              </div>
              {filteredStats
                ...
                .map((s, i) => {
                  ...
                  return (
                    <div
                      key={s.playerId}
                      className={cn(
                        "grid grid-cols-[32px_1fr_32px_32px_32px_32px_36px_44px_44px] gap-0 px-2 py-2.5 text-xs items-center",
